...
class XZA_小锥度凹_UI:
    def __init__(self, parent, on_execute, font_size=12):
        ...
        self.param_file = os.path.join(config_dir, "XZA_小锥度凹_params.json")

    def create_ui(self, frame):
        f_large = ("Segoe UI", self.font_size, "bold")
        f_norm = ("Segoe UI", self.font_size)

        ttk.Label(frame, text="小锥度凹模参数输入", font=f_large).pack(pady=(0, 20))
    ...
    def _on_run(self):
        try:
            ...
            execute_lisp(None, "小锥度凹模", "XZA_小锥度凹.lsp", "xza", values, _build_args, TEMPLATE_PATH, LSP_DIR)
        except Exception as e:
