...
class JZM_短尾M24_基准模_UI:
    def __init__(self, parent, on_execute: Callable, font_size: int = 12):
        ...
        self.inputs = {}
        # 修正路径处理：使用用户主目录下的配置目录，防止权限问题
        self.config_dir = os.path.join(os.path.expanduser("~"), ".autolisp_mgr")
        os.makedirs(self.config_dir, exist_ok=True)
        self.param_file = os.path.join(self.config_dir, "JZM_短尾M24_基准模_params.json")
    ...
    def create_ui(self, frame):
        ...
        ttk.Label(scale_frame, text="图纸比例:", width=15, font=f_norm).pack(side=tk.LEFT)
        ...
        ttk.Label(slot_frame, text="开槽选项:", width=15, font=f_norm).pack(side=tk.LEFT)
        ...
        # 核心：在 UI 组件全部初始化并加入 inputs 字典后，再加载保存的参数
        self.load_params()
