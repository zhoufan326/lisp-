...
class DWT_短尾凸_UI:
    def __init__(self, parent, on_execute, font_size=12):
        ...
        self.param_file = os.path.join(config_dir, "DWT_短尾凸_params.json")

    def create_ui(self, frame):
        f_large = ("Segoe UI", self.font_size, "bold")
        f_norm = ("Segoe UI", self.font_size)

        ttk.Label(frame, text="短尾凸模参数输入", font=f_large).pack(pady=(0, 20))
    ...
    def _on_run(self):
        try:
            ...
            execute_lisp(None, "短尾凸模", "DWT_短尾凸.lsp", "dwt", values, _build_args, TEMPLATE_PATH, LSP_DIR)
        except Exception as e:
